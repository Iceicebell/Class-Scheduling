...
class Solution:
    ...
    def add_course_assignment(self, course_id, day, start_hour, duration, course_code, course_block, cursor):
        ...
        for conflicting_course in conflicting_courses:
            ...
            for assignment in conflicting_assignments:
                ...
                if day == conflict_day and (start_hour < conflict_end and start_hour + max_duration > conflict_start):
                    # Conflict detected, adjust start_hour to avoid conflict
                    start_hour = conflict_end
                    if start_hour + max_duration > 20:
                        start_hour = 20 - max_duration

        # Ensure no overlap with existing assignments for the same course
    ...
    def calculate_fitness(self, cursor):
        if self.fitness_score is None:
                                ...
                                for assignment2 in assignments2:
                                    ...
                                    if day1 == day2:
                                        if start_hour1 < start_hour2 < end_time1 or start_hour1 < end_time2 < end_time1:
                                            conflicts_penalty += Decimal(4000)
            ...
            for course_id, assignments in self.schedule.items():
                morning_slots_filled = sum(Decimal(1) for _, start_hour, _, _, _ in assignments if start_hour < 12)
                afternoon_slots_filled = sum(Decimal(1) for _, start_hour, _, _, _ in assignments if start_hour >= 13)
...
def mutate(self, cursor):
    ...
    if len(assignments) > 1:
        ...
        assignment_index = 0  # Ensure assignment_index is always assigned
